...
<table class="course-table" width="75%" align="center" border>
	...
	<%
	try{
		String sql = "SELECT c.c_id, c.c_id_no, c.c_name, c.c_unit, t.t_time, t.t_max, p.p_name FROM course c, teach t, professor p WHERE (c.c_id, c.c_id_no) IN (SELECT c_id, c_id_no FROM enroll WHERE s_id ='" + session_id + "') AND t.c_id = c.c_id AND t.c_id_no = c.c_id_no AND t.p_id = p.p_id ORDER BY c.c_id, t.t_time";
		...
		while(rs.next()){
			...
			<tr>
				...
				<td align="center"><a href="delete_verify.jsp?c_id=<%=rs.getString(1) %>&c_id_no=<%=rs.getInt(2) %>">삭제</a></td>
			</tr>
		<%
	}
